...
function validate(data) {
  ...
  if (!rule) errors.push("活動類型無效，請從清單選擇");
  ...
  if (rule && rule.needId) {
    if (!TW_ID_RE.test(data.idNumber || "")) errors.push("身分證字號格式錯誤（須為 1 個英文字母加 9 個數字，例：A123456789）");
  }
  ...
  if (!Number.isFinite(amount) || amount <= 0) {
    ...
    if (rule.fixed && amount !== rule.amount) errors.push(`「${data.activityType}」金額應為 ${rule.amount} 元`);
    if (!rule.fixed && (amount < rule.min || amount > rule.max)) {
      errors.push(`「${data.activityType}」金額需介於 ${rule.min}～${rule.max} 元之間`);
    }
  }
